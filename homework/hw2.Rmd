...
sample_var_with_question_mark has a negative value in this specific case with seed 140778.
sample_var_with_question_mark varies over different seeds, and can take both positive and negative values. 
The estimate makes no sense because we anticipate a sample variance of 1.  

$$\begin{aligned}
\frac{1}{n} \sum_{i=1}^n X_i^2 &= \frac{1}{n} \sum (1 \times 10^{20} + 2 \times 10^{10} \epsilon_i + \epsilon_i^2)\\
\frac{1}{n} \sum_{i=1}^n (X_i^2)_{machine} &\approx \frac{1}{n} \sum (1 \times 10^{20} + 2 \times 10^{10} \epsilon_i) \;\text{due to limitation in precision, the small error squared quantity is truncated}\\

(\frac{1}{n} \sum_{i=1}^n X_i)^2 &= (1 \times 10^{10} + \frac{1}{n} \sum_{i=1}^n \epsilon_i)^2 \\
(\frac{1}{n} \sum_{i=1}^n X_i)^2_{machine} &\approx 1 \times 10^{20} + 2\times 10^{10} \bar{\epsilon} \; \text{epsilon bar squared term is again truncated}  
\end{aligned}$$

Due to the truncation in both the sample_sec_mom and sample_mean, we will never obtain the true value of sample variance. When we tune down the non-random portion of theta to a smaller value (e.g. 1e5) we obtain the anticipated estimated sample variance of around 1. 
...
The relative error plot starts with large error for large finite difference step sizes. The relative error decreases with decreasing finite difference step size , which is reasonable because with finer step we will better approximate the true derivative. After step sizes of around 2^-20, the relative error begins to rise again, most likely due to precision limitation. The slope in the "latter part" is approximately -1 (log2 relative error/log2 stepsize), suggesting that we are hitting the limit of machine precision and every extra decimal digit we demand the approximation just gets truncated somewhere in the calculation and translates to error. At some extremely small finite difference value the relative error should hit a boundary and flatten out after that, as x + dx and x - dx becomes no different to the computer and func(x + dx) - func(x - dx) just becomes 0.  
...
gaussian_grad <- function(x, Sigma_inv) {
  ...
  # t(A) %*% A %*% x seems to be the gradient for ||Ax||2
  grad <- (-t(A) %*% A %*% x)
  return(grad)
}
...
https://github.com/wli51/hiperglm/pull/1
